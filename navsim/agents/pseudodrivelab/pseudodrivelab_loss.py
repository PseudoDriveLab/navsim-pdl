...
from navsim.agents.pseudodrivelab.pseudodrivelab_features import BoundingBox2DIndex

@torch.no_grad()
def mask_blind_site(config, span: int = 30):
    # EDITTED: Pseudolab by Kim.
    # Valid area : (90-range) deg ~ (90+range) deg
    # TODO: Make the mask as a constant
    width = config.bev_pixel_width
    height = config.bev_pixel_height
    ...
    for i in range(width):
        for j in range(height):
            ...
            if (90-span) < angle <= (90+span):
                mask[i, j] = True
    
    return mask

def pseudodrivelab_loss(targets: Dict[str, torch.Tensor], predictions: Dict[str, torch.Tensor], config: PseudodrivelabConfig):
    """
    Helper function calculating complete loss of Pseudolab
    :param targets: dictionary of name tensor pairings
    :param predictions: dictionary of name tensor pairings
    :param config: global Pseudolab config
    ...
    # EDITTED: Pseudolab by Kim.
    # Valid area : 45 deg ~ 135 deg
    BEV_BLIND = -1
    bev_semantic_map = targets["bev_semantic_map"].long() # NOTE: Pseudo by Kim. (BatchSize, bev_pixel_width, bev_pixel_height)
    mask = mask_blind_site(config)

    # Check shapes
    # assert bev_semantic_map.shape == mask.shape, f"Shape mismatch: {bev_semantic_map.shape} vs {mask.shape}"
    try:
        # bev_semantic_map[mask] = BEV_BLIND
        for i in range(bev_semantic_map.shape[0]):
            values = bev_semantic_map[i,:,:]
            values[mask] = BEV_BLIND
            bev_semantic_map[i,:,:] = values
    except Exception as e:
        assert False, f"{e}\n{i}\n{values.shape} {mask.shape}\n{bev_semantic_map[i-1,-10:,:10]}\n{bev_semantic_map[0,-10:,:10]}"

    bev_semantic_loss = F.cross_entropy(predictions["bev_semantic_map"], bev_semantic_map, ignore_index=BEV_BLIND)
    loss = (
...
def _agent_loss(targets: Dict[str, torch.Tensor], predictions: Dict[str, torch.Tensor], config: PseudodrivelabConfig):
    """
