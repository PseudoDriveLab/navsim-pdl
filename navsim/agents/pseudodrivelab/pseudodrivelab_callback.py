...
class PseudodrivelabCallback(pl.Callback):
    """Visualization Callback for Pseudolab during training."""
    ...
    def _visualize_model(
        ...
        """
        Create tile of input-output visualizations for Pseudolab.
        ...
        """
        # EDITTED: Psuedolab by Kim.
        # Visualization on Tensorboard is modified.
        try:
            camera = features["camera_feature"].permute(0, 2, 3, 1).numpy()
            bev = targets["bev_semantic_map"].numpy()
            lidar_map = bev         # Not using lidar_map
            ...
            for sample_idx in range(self._num_rows * self._num_columns):
                ...
                start_w = 0
                plot[cam_h//2:, start_w:start_w + bev_w] = semantic_map_to_rgb(bev[sample_idx], self._config)
                start_w = (cam_w//2) // 3 * 1
                plot[cam_h//2:, start_w:start_w + bev_w] = semantic_map_to_rgb(pred_bev[sample_idx], self._config)
                ...
                start_w = (cam_w//2) // 3 * 2
                plot[cam_h//2:, start_w:start_w + bev_w] = lidar_map_to_rgb(
...
def semantic_map_to_rgb(semantic_map: npt.NDArray[np.int64], config: PseudodrivelabConfig) -> npt.NDArray[np.uint8]:
    """
